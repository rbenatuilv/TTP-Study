...
# opponent[t, s] <= N - 1 <-> is_home[t, s] = 1
for t in teams:
    for s in slots:
...
for t in teams:
    for j1 in teams_duplicated:
        if j1 <= N - 1:
            ...
            model.Add(init_travel[t] == distancia[t][j1 - N]).OnlyEnforceIf(auxiliar[t, j1, slots[0]])
            model.Add(last_travel[t] == distancia[t][j1 - N]).OnlyEnforceIf(auxiliar[t, j1, slots[-1]])
        ...
        for j1 in teams_duplicated:
            for j2 in teams_duplicated:
                ...
                if j1 <= N - 1 and j2 <= N - 1: 
                    model.Add(travel[t, s] == 0).OnlyEnforceIf(auxiliar[t, j1, s]).OnlyEnforceIf(auxiliar[t, j2, s + 1])
                elif j1 <= N - 1 and j2 >= N:
                    model.Add(travel[t, s] == distancia[t][j2 - N]).OnlyEnforceIf(auxiliar[t, j1, s]).OnlyEnforceIf(auxiliar[t, j2, s + 1])
                elif j1 >= N and j2 <= N - 1:
                    model.Add(travel[t, s] == distancia[j1 - N][t]).OnlyEnforceIf(auxiliar[t, j1, s]).OnlyEnforceIf(auxiliar[t, j2, s + 1])
                elif j1 >= N and j2 >= N:
                    model.Add(travel[t, s] == distancia[j1 - N][j2 - N]).OnlyEnforceIf(auxiliar[t, j1, s]).OnlyEnforceIf(auxiliar[t, j2, s + 1])

funcion_objetivo = model.NewIntVar(0, 13800 * N, name='objective')

model.Add(funcion_objetivo == sum(travel[t, s] for t in teams for s in slots) + sum(init_travel[t] + last_travel[t] for t in teams))
