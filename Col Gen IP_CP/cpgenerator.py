...
class CPPatternGenerator:
    ...
    def set_vars(self, model):
        ...
        self.pat_hash = model.NewIntVar(0, self.N ** (self.S) - 1,
                                        'pat_hash')
    ...
    def set_constrs(self, home, model):
        ...
        for s in self.slots:
            ...
            model.Add(self.hash_aux[s] == self.opponent[s]).OnlyEnforceIf(self.is_home[s].Not())

        model.Add(self.pat_hash == sum((self.hash_aux[s] - self.N) * self.N ** (s - 1)
                                       for s in self.slots))

        for ph in self.patt_hashes:
    ...
    def single_solve(self, home, pi):
        ...
        if status == cp_model.OPTIMAL:
            ...
            self.patt_hashes.append(self.solver.Value(self.pat_hash))
        else:
